...
<div class="card">
    <div class="card-header">
        <h2 class="card-title">🛒 My Orders (Purchases from Factories)</h2>
    </div>
...
<div class="card">
        ...
        <div style="border: 1px solid rgba(255,255,255,0.1); border-radius: 12px; padding: 1rem; margin-bottom: 1rem;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.75rem;">
                ...
                <div style="text-align: right;">
                    ...
                    <div>From: <strong>{{ $order->sellerFactory->name ?? 'Unknown Factory' }}</strong></div>
                </div>
            </div>
            
            @if($order->status === 'pending')
            <form action="{{ route('distributor.orders.edit', $order) }}" method="POST">
                @csrf
                @method('PATCH')
            ...
            <!-- Hidden cancel form -->
            <form id="cancel-form-{{ $order->id }}" action="{{ route('distributor.orders.cancel', $order) }}" method="POST" style="display: none;">
                @csrf
                @method('DELETE')
            </form>
            @else
    ...
    @else
        <p style="color: rgba(255,255,255,0.5); text-align: center; padding: 2rem;">
            No orders yet. Visit the <a href="{{ route('distributor.marketplace') }}" style="color: #22c55e;">Marketplace</a> to place orders.
        </p>
    @endif
</div>
@endsection
